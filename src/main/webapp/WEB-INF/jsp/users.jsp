        ...
        <div class="row">
            <c:forEach var="user" items="${requestScope.users}">
                <div class="col-sm-6">
                    <div class="card">
                        <div class="card-body">
                            <form action="${pageContext.request.contextPath}/controller?command=update_role"
                                  method="post">
                                <h5 class="card-title"><strong><ut:locale_tag key="name"/>:</strong> ${user.name}</h5>
                                <h5 class="card-title"><strong><ut:locale_tag key="users.role"/>:</strong> ${user.role}
                                </h5>
                                <input type="hidden" name="userId" value="${user.id}"/>
                                <button class="btn btn-primary"><ut:locale_tag key="users.update-role"/></button>
                            </form>
                            <form action="${pageContext.request.contextPath}/controller?command=rollback_role"
                                  method="post">
                                <input type="hidden" name="userId" value="${user.id}"/>
